...
func TestSecretReplicator(t *testing.T) {
	...
	t.Run("replication is pushed to other namespaces generating ownerReferences", func(t *testing.T) {
		sourceLabels := map[string]string{
			"foo":   "bar",
			"hello": "world",
		}
		source := corev1.Secret{
			...
			ObjectMeta: metav1.ObjectMeta{
				...
				Annotations: map[string]string{
					common.ReplicateTo:             prefix + "test2",
					common.GenerateOwnerReferences: "true",
				},
				Labels: sourceLabels,
				OwnerReferences: []metav1.OwnerReference{{
					APIVersion: "v1",
		...
		updSource, err := secrets.Create(context.TODO(), &source, metav1.CreateOptions{})
		require.NoError(t, err)
		...
		require.Equal(t, updSource.UID, updTarget.OwnerReferences[0].UID)
		require.Equal(t, updSource.APIVersion, updTarget.OwnerReferences[0].APIVersion)
		require.Equal(t, updSource.Kind, updTarget.OwnerReferences[0].Kind)
		require.Equal(t, updSource.Name, updTarget.OwnerReferences[0].Name)
